...
"#,
    ...
    en.insert(
        "starting_bio_pacing_track",
        "Starting BIO pacing track for {}...",
    );
    ...
    en.insert(
        "manual_bio_fetch_triggered",
        "Manual BIO fetch triggered for: {}",
    );
    en.insert(
        "manual_bio_fetch_success",
        "Successfully fetched BIO for: {} ({})",
    );
    en.insert(
        "manual_bio_fetch_invalid_input",
        "Invalid input. If you want to fetch a user, paste a valid VRChat User ID (e.g., usr_xxx).",
    );
    ...
    en.insert("user_report_timeline_title", "# Timeline");
    en.insert("user_report_query_range", "Query Range");
    en.insert("user_report_last_30_days", "Last 30 days");
    en.insert("user_report_generated_at", "Generated At");
    en.insert(
        "user_report_no_timeline",
        "No VRCX timeline data found for the last 30 days.",
    );
    en.insert("user_report_table_time", "Date Time");
    en.insert("user_report_table_type", "Type");
    en.insert("user_report_table_detail", "Detail");
    en.insert(
        "manual_report_fetch_triggered",
        "Manual VRCX report generation triggered for: {}",
    );
    en.insert(
        "manual_report_fetch_success",
        "Generated VRCX report for {}: {}",
    );
    en.insert(
        "manual_report_fetch_failed",
        "Failed to generate VRCX report for {}: {}",
    );
    en.insert(
        "manual_report_fetch_invalid_input",
        "Invalid input. Paste a valid VRChat User ID (for example: usr_xxx).",
    );
...
"#,
    ...
    zh.insert(
        "manual_bio_fetch_invalid_input",
        "无效输入。如果要获取用户，请粘贴有效的 VRChat 用户 ID（例如 usr_xxx）。",
    );
    ...
    zh.insert("user_report_timeline_title", "# 时间线");
    zh.insert("user_report_query_range", "查询范围");
    zh.insert("user_report_last_30_days", "最近 30 天");
    zh.insert("user_report_generated_at", "生成时间");
    zh.insert(
        "user_report_no_timeline",
        "最近 30 天内未找到该用户的 VRCX 时间线数据。",
    );
    zh.insert("user_report_table_time", "日期时间");
    zh.insert("user_report_table_type", "类型");
    zh.insert("user_report_table_detail", "详细内容");
    zh.insert(
        "manual_report_fetch_triggered",
        "手动触发 VRCX 报告生成: {}",
    );
    zh.insert("manual_report_fetch_success", "已生成 {} 的 VRCX 报告: {}");
    zh.insert("manual_report_fetch_failed", "生成 {} 的 VRCX 报告失败: {}");
    zh.insert(
        "manual_report_fetch_invalid_input",
        "输入无效。请粘贴有效的 VRChat 用户 ID（例如 usr_xxx）。",
    );
...
"#,
    ...
    ja.insert(
        "vrchat_capture_read_error",
        "VRChatキャプチャ読み取りエラー: {}",
    );
    ja.insert(
        "mic_capture_read_error",
        "マイクキャプチャ読み取りエラー: {}",
    );
...
"#,
    ...
    ko.insert(
        "bio_already_exists",
        "{} ({})의 BIO가 이미 로컬에 존재하므로 가져오기를 건너뜁니다.",
    );

...
impl LogWatcher {
    ...
    fn get_latest_log_file(&self) -> Option<PathBuf> {
        ...
        // Optimization: Use max_by_key instead of sort_by to achieve O(N) complexity
        // and reduce redundant metadata() calls.
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Optimization: Quick heuristic check for keywords to avoid parsing timestamps
        // on irrelevant log lines (which are the majority).
        if !line.contains("[Behaviour]") && !line.contains("uSpeak") && !line.contains("] Joining ")
        {
            return None;
        }

        // Optimization: Use lazy evaluation for timestamp to ensure it's only computed
        // if a relevant log event is found.
        let get_ts = || Self::parse_timestamp(line);
    ...
    fn read_new_lines(&mut self, tx: &mpsc::UnboundedSender<LogEvent>) {
        ...
        // Optimization: Use a peekable iterator to process all lines except the final one
        // if the buffer doesn't end in a newline, avoiding unnecessary Vec allocation.
        let has_trailing_newline = content.ends_with('\n') || content.ends_with('\r');
        let mut lines_it = content.lines().peekable();

        while let Some(line) = lines_it.next() {
            if !has_trailing_newline && lines_it.peek().is_none() {
                // Save the incomplete final line for next read
...
mod tests {
    ...
    fn test_parse_timestamp() {
        let line = "2026.03.08 01:14:09 [Behaviour] ...";
    ...
    fn test_parse_line_instance() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:09 [Behaviour] Joining wrld_12345:6789";
        let event = watcher.parse_line(line).unwrap();
        ...
        {
            assert_eq!(location, "wrld_12345:6789");
            assert_eq!(timestamp, "2026-03-08T01:14:09");
        } else {
            panic!("Wrong event type");
        }
    ...
    fn test_parse_line_join() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:09 [Behaviour] OnPlayerJoined User (usr_1234)";
        let event = watcher.parse_line(line).unwrap();
    ...
    fn test_parse_line_left() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:09 [Behaviour] OnPlayerLeft User (usr_1234)";
        let event = watcher.parse_line(line).unwrap();
    ...
    fn test_read_new_lines_incomplete() {
        ...
        fs::write(&log_path, "2026.03.08 01:14:09 [Behaviour] Entering Room: World\n2026.03.08 01:14:10 [Behaviour] OnPlayerJoined User").unwrap();

        watcher.current_log_file = Some(log_path.clone());
        watcher.read_new_lines(&tx);

        // Should have 1 event (World), and 1 incomplete line
        ...
        assert_eq!(
            ...
            "2026.03.08 01:14:10 [Behaviour] OnPlayerJoined User"
        );

        // Append the rest
        let mut file = fs::OpenOptions::new().append(true).open(&log_path).unwrap();
        use std::io::Write;
        writeln!(file, " (usr_1234)").unwrap();

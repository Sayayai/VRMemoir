...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Optimization: early return if the line is not relevant
        if !line.contains("[Behaviour]") && !line.contains("uSpeak") {
            return None;
        }

        // Optimization: Lazy timestamp parsing via closure to avoid it on irrelevant lines
...
mod tests {
    ...
    fn test_parse_line_irrelevant() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:09 [Other] Some random log";

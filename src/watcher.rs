...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Optimization: Early return for lines that don't contain our keywords.
        // Most log lines are irrelevant (GC, physics, uSpeak noise, etc).
    ...
    fn read_new_lines(&mut self, tx: &mpsc::UnboundedSender<LogEvent>) {
        ...
        // Optimization: Process lines using a peekable iterator instead of collecting into a Vec.
        ...
        while let Some(line) = lines_iter.next() {
            if lines_iter.peek().is_none() && !has_trailing_newline {
                // This is the last line and it's incomplete

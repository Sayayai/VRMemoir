...
impl LogWatcher {
    ...
    async fn get_latest_log_file(&self) -> Option<PathBuf> {
        if tokio::fs::metadata(&self.log_dir).await.is_err() {
            return None;
        }
            ...
            if name_str.starts_with("output_log_") && name_str.ends_with(".txt") {
                if let Ok(meta) = entry.metadata().await {
                    if let Ok(modified) = meta.modified() {
                        match latest_file {
                            None => latest_file = Some((entry.path(), modified)),
    ...
    pub(crate) fn parse_timestamp(line: &str) -> String {
    ...
    pub(crate) fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Early returns for irrelevant lines
...
mod tests {
    ...
    fn test_parse_timestamp() {
        let line = "2026.03.08 01:14:09 Log      -  [Behaviour] Entering Room: Test World";
    ...
    fn test_parse_line_player_joined() {
        let watcher = LogWatcher::new();
        let line =
            "2026.03.08 01:14:09 Log      -  [Behaviour] OnPlayerJoined Display Name (usr_123-abc)";
        let event = watcher.parse_line(line).unwrap();
        ...
        } = event
        {
            ...
            assert_eq!(user_id, Some("usr_123-abc".to_string()));
            assert_eq!(timestamp, "2026-03-08T01:14:09");
        } else {
            panic!("Wrong event type");
        }
    ...
    fn test_parse_line_player_left() {
        let watcher = LogWatcher::new();
        let line =
            "2026.03.08 01:14:09 Log      -  [Behaviour] OnPlayerLeft Display Name (usr_123-abc)";
        let event = watcher.parse_line(line).unwrap();

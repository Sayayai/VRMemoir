...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Early return heuristics: most VRChat log lines are noise (e.g., debug or network logs).
        // By skipping lines that don't contain these key strings, we avoid expensive processing.
        if !line.contains("[Behaviour]") && !line.contains("uSpeak") {
            return None;
        }

        // Lazy timestamp parsing: only compute if we actually find an event.
        let parse_ts = || Self::parse_timestamp(line);

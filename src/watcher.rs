...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        ...
        if let Some(idx) = line.find("] Joining wrld_") {
            // "Joining " is 8 chars, but "wrld_" is part of the ID.
            // In the original code it was split("] Joining ").nth(1)
            // "] Joining " is 10 chars.
            if let Some(joining_idx) = line.find("] Joining ") {
                let location = &line[joining_idx + 10..];
...
mod tests {
    ...
    fn test_parse_line_player_joined() {
        ...
        let line = "2026.03.08 01:14:09 [Behaviour] OnPlayerJoined Bolt (usr_1234-abcd)";
        let event = watcher.parse_line(line).unwrap();
        match event {
            LogEvent::PlayerJoined {
                ...
                assert_eq!(user_id, Some("usr_1234-abcd".to_string()));
                assert_eq!(timestamp, "2026-03-08T01:14:09");
            }

...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Early return for most common log lines to avoid expensive checks
        if !line.contains("[Behaviour]") && !line.contains("uSpeak") {
            return None;
        }

        // Lazy timestamp parsing: only compute if we actually find an event
        ...
        // 2. Instance ID
        const JOINING_WRLD: &str = "] Joining ";
        if let Some(idx) = line.find(JOINING_WRLD) {
            if line[idx + JOINING_WRLD.len()..].starts_with("wrld_") {
                let location = &line[idx + JOINING_WRLD.len()..];
    ...
    fn read_new_lines(&mut self, tx: &mpsc::UnboundedSender<LogEvent>) {
        ...
        while let Some(line) = lines_iter.next() {
            // If this is the last line and it's incomplete, save it for next read

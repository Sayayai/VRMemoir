...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Early heuristic: skip lines that definitely don't contain our events
        // Most VRChat log lines are boilerplate or unrelated to tracking
        if !line.contains("[Behaviour]") && !line.contains("uSpeak") {
            return None;
        }

        // Lazy timestamp parsing: only parse when we know it's an event we care about.

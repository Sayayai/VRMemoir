...
static PLAYER_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(.+) \((usr_[a-f0-9-]+)\)").unwrap());
...
impl LogWatcher {
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        // Optimization: Early return for lines that definitely don't contain events we care about
        if !line.contains("[Behaviour]") && !line.contains("uSpeak") {
            return None;
        }

        // Lazy timestamp parsing: only parse it if we actually find an event
        ...
        if let Some(pos) = line.find("] OnPlayerJoined ") {
            ...
            if let Some(caps) = PLAYER_RE.captures(parts) {
...
mod tests {
    ...
    fn test_parse_line_location() {
        ...
        let line = "2026.03.08 01:14:09 Log      -  [Behaviour] Entering Room: The Black Cat";
    ...
    fn test_parse_line_player_joined() {
        let watcher = LogWatcher::new();
        let line =
            "2026.03.08 01:14:10 Log      -  [Behaviour] OnPlayerJoined Bolt (usr_1234-5678)";
        let event = watcher.parse_line(line).unwrap();
    ...
    fn test_parse_line_player_left() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:11 Log      -  [Behaviour] OnPlayerLeft Bolt (usr_1234-5678)";
        let event = watcher.parse_line(line).unwrap();
    ...
    fn test_parse_line_irrelevant() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:12 Log      -  Some random log message";

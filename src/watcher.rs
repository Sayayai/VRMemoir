...
impl LogWatcher {
    ...
    fn parse_timestamp(line: &str) -> String {
        ...
        if line.len() >= 19 && line.is_char_boundary(19) && line.as_bytes()[0].is_ascii_digit() {
            ...
            for (i, &byte) in line.as_bytes().iter().enumerate().take(19) {
                match i {
                    4 | 7 => s.push('-'),
                    10 => s.push('T'),
    ...
    fn parse_line(&self, line: &str) -> Option<LogEvent> {
        ...
        if let Some(idx) = line.find("] Joining wrld_") {
            // idx points to "]" of "] Joining wrld_"
            // Marker is "] Joining " (10 chars)
    ...
    fn read_new_lines(&mut self, tx: &mpsc::UnboundedSender<LogEvent>) {
        ...
        while let Some(line) = lines_it.next() {
            if lines_it.peek().is_none() && !has_trailing_newline {
...
mod tests {
    ...
    fn test_parse_line_world_name() {
        ...
        let line = "2026.03.08 01:14:09 [Behaviour] Entering Room: Home";
        if let Some(LogEvent::Location { world_name, timestamp }) = watcher.parse_line(line) {
    ...
    fn test_parse_line_player_joined() {
        ...
        if let Some(LogEvent::PlayerJoined { display_name, user_id, timestamp }) = watcher.parse_line(line) {
            assert_eq!(display_name, "Bolt");
            assert_eq!(user_id, Some("usr_1234-5678".to_string()));
            assert_eq!(timestamp, "2026-03-08T01:14:09");
        } else {
            panic!("Failed to parse player joined");
        }
    }
}

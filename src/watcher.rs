...
impl LogWatcher {
    ...
    fn parse_timestamp(line: &str) -> String {
        ...
        if line.len() >= 19 && line.is_char_boundary(19) && line.as_bytes()[0].is_ascii_digit() {
            ...
            for (i, &byte) in b.iter().take(19).enumerate() {
                if i == 4 || i == 7 {
                    s.push('-');
                } else if i == 10 {
                    s.push('T');
...
mod tests {
    ...
    fn test_parse_timestamp() {
        ...
        let short_line = "2026.03.08";
        assert_eq!(LogWatcher::parse_timestamp(short_line), "");

        let non_numeric = "abcd.ef.gh ij:kl:mn [Behaviour]";
        assert_eq!(LogWatcher::parse_timestamp(non_numeric), "");
    }
    ...
    #[test]
    fn test_parse_line_location_instance() {
        let watcher = LogWatcher::new();
    ...
    fn test_parse_line_player_joined() {
        ...
        // With User ID
        let line = "2026.03.08 01:14:09 [Behaviour] OnPlayerJoined Alice (usr_c1644b11-aa4c-4103-9993-27c9d7494519)";
        let event = watcher.parse_line(line).unwrap();
        ...
        } = event
        {
            ...
            assert_eq!(user_id, Some("usr_c1644b11-aa4c-4103-9993-27c9d7494519".to_string()));
            assert_eq!(timestamp, "2026-03-08T01:14:09");
        } else {
            panic!("Expected PlayerJoined event");
        }

        // Without User ID (though rare in modern VRChat logs for this specific event)
        let line_no_id = "2026.03.08 01:14:09 [Behaviour] OnPlayerJoined Alice";
        let event = watcher.parse_line(line_no_id).unwrap();
        if let LogEvent::PlayerJoined {
            display_name,
            user_id,
            ..
    ...
    fn test_parse_line_player_left() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:09 [Behaviour] OnPlayerLeft Alice (usr_c1644b11-aa4c-4103-9993-27c9d7494519)";
        let event = watcher.parse_line(line).unwrap();
    ...
    #[test]
    fn test_parse_line_voice_ready() {
        let watcher = LogWatcher::new();
        let line = "2026.03.08 01:14:09 uSpeak Start Microphone";
        let event = watcher.parse_line(line).unwrap();
        if let LogEvent::VoiceReady { timestamp } = event {
            assert_eq!(timestamp, "2026-03-08T01:14:09");
        } else {
            panic!("Expected VoiceReady event");
        }
    }
}
